...
## 8. Стать администратором

1. Зайти на задеплоенный сайт, зарегистрироваться
2. Открыть «Личный кабинет» → нажать «Стать администратором»

Сработает только для первого нажавшего (пока документ `settings/admins` пуст — см. `firestore.rules` и `client/src/context/AuthContext.jsx`). Дальше в разделе «Админка» можно наполнять каталог программ.
